...
import type {
	LavaSearchFilteredResponse, LavaSearchType, LavaSrcSearchPlatformBase,
	SearchResult
...
export default {
    ...
    autocomplete: async (client, interaction) => {
        ...
        // get source from options if it's undefined it doesn't matter, default from manager options will be used.
        // if a url is used, it doesn't matter what the source is.
        const src = interaction.options.getString("source") as LavaSrcSearchPlatformBase;
        ...
        const res = await player.lavaSearch({ query: focussedQuery, types: [type], source: src }, interaction.user);
