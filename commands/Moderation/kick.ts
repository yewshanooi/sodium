...
export default {
	...
	execute: async (client, interaction) => {
		...
		if (!interaction.member.permissions.has(PermissionsBitField.Flags.KickMembers)) return interaction.reply({ embeds: [client.errors.noPermission] });

			const userField: any = interaction.options.getMember('user');
		...
		try {
			...
			await userField.send(`Hi **${userField.user.username}!**, you have been kicked from the server **${interaction.guild.name}**. ${reasonField ? "\nReason: " + reasonField : ""}`);
		} catch (err) {
			console.error(err);
		}

		return interaction.reply({ embeds: [embed] }).then(() => {
			userField.kick(userField);
		});
	}
} as Command;