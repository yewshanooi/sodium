...
const { isLink, isPrivate, steamProfileName, newUser } = require('../../utils/helpers');
...
module.exports = {
    ...
    async execute (interaction, client) {
		...
		if (sub === 'link') {
			...
			const saved = await newUser(steamid, interaction.user.id, interaction.guild.id);
		...
		try {
			...
			const resolved = await isLink({ steamIdOrUsername: optIdOrUsername, interaction, apiKey, guildId: interaction.guild.id });
			...
			if (await isPrivate(profileData.response)) {
			...
			let gamesRes = await fetch(gamesURL);
			...
			let recentRes = await fetch(recentURL);
