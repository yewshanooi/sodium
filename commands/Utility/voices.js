  ...
  async execute (interaction, client) {
    ...
    try {
      if (searchQuery) {
        ...
        if (models.size === 0) {
          return interaction.editReply({ content: "Don't found voices with that name.", ephemeral: true });
        }
      } else {
      ...
      console.error(error);
      await interaction.editReply({ content: 'Hubo un error al buscar las voces.', ephemeral: true });
    }
  },
};