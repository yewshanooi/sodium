...
module.exports = {
    ...
    async execute (interaction) {
        ...
        const queryField = interaction.options.getString('message');
            ...
            if (!process.env.BRAINSHOP_BID) return interaction.reply({ embeds: [global.errors[1]], ephemeral: true });
            if (!process.env.BRAINSHOP_API_KEY) return interaction.reply({ embeds: [global.errors[1]], ephemeral: true });
