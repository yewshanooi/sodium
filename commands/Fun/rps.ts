        ...
        collector.on('collect', async collected => {
            if (collected.customId === 'Paper' && botOption === 'Rock' || collected.customId === 'Paper' && botOption === 'Scissors' || collected.customId === 'Scissors' && botOption === 'Rock') {
                const userLostEmbed = new EmbedBuilder()
                    ...
                    .setColor(client.embedColor as any);
                await collected.update({ embeds: [userLostEmbed], components: [buttons] });
                collector.stop();
                ...
                const tieEmbed = new EmbedBuilder()
                    ...
                    .setColor(client.embedColor as any);
                await collected.update({ embeds: [tieEmbed], components: [buttons] });
                collector.stop();
            } else {
                const userWonEmbed = new EmbedBuilder()
                    ...
                    .setColor(client.embedColor as any);
                await collected.update({ embeds: [userWonEmbed], components: [buttons] });
                collector.stop();
                }
            });
