...
export default {
	...
	execute: async (client, interaction) => {
		...
		if (!process.env.RIOTGAMES_API_KEY) return interaction.editReply({ embeds: [client.errors.noAPIKey] });

		const gamename = interaction.options.getString('username');
		const tagline = interaction.options.getString('tag');
		const region = interaction.options.getString('region');

		const RiotAccount: any = await fetch(`https://asia.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${gamename}/${tagline}?api_key=${process.env.RIOTGAMES_API_KEY}`)
			.then(res => res.json());

			if (!RiotAccount.puuid) return interaction.editReply({ content: 'Error: No such Riot ID found.' });

		const League: any = await fetch(`https://${region}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${RiotAccount.puuid}?api_key=${process.env.RIOTGAMES_API_KEY}`)
			.then(res => res.json());
